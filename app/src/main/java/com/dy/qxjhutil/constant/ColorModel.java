package com.dy.qxjhutil.constant;

/**
 * Created by hbysd on 2019/1/20.
 */

public class ColorModel {
    public static final int color_善 = 0xff52DFCE;
    public static final int color_恶 = 0xff9C61AD;
    public static final int color_守序 = 0xff52DFCE;
    public static final int color_混乱 = 0xffDE7173;
//    public static final int color_所得 = 0xffB23AEE;

    public static final int TYPE_PARENT_1 = 0xffffffff;
    public static final int TYPE_PARENT_2 = 0xfffc8d96;
    public static final int TYPE_PARENT_3 = 0xffFD9DFA;
    public static final int TYPE_PARENT_4 = 0xff9bd55a;
    public static final int TYPE_PARENT_5 = 0xff59A6EE;


    public static final int TYPE_BG_RED = 0xffFC8D95;
    public static final int TYPE_BG_GREEN = 0xffCBF382;
    public static final int TYPE_BG_DEFAULT = 0xffffffff;


    public static final int TYPE_COLOR_RED = 0xffdf1c2a;
    public static final int TYPE_COLOR_GREEN = 0xff87cc11;
    public static final int TYPE_COLOR_DEFAULT = 0xff212832;


}
